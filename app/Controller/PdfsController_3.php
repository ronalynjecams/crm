        ...
        foreach ($quote_products as $quote_prod) {
            ...
            foreach ($quote_prod['QuotationProductProperty'] as $desc) {
                if (is_null($desc['property'])) {
                    $tmp = ucwords($desc['ProductProperty']['name']);
                    if($tmp == "Material" || $tmp == "Materials") {
                        $ppn = "Materials (General Info)";
                    }
                    if($tmp == "Dimension" || $tmp == "Dimensions") {
                        $ppn = "Dimension Info";
                    }
                    
                    if($tmp!="Made In") {
                        $prod_prop[] = '<li class="list-group-item"> <font style="font-weight:bold">' . $ppn . '</bold> : ' . ucwords($desc['ProductValue']['value']) . '</li>';
                    }
                } else {
                    $prod_prop[] = '<li class="list-group-item"> ' . ucwords($desc['property']) . ' : ' . ucwords($desc['value']) . '</li>';
                }
...
//            }
            ...
            $img = $this->thumbnail($prod['ProductCombo']['Product']['image']);
			// $data[$count][$key] = '<img class="img-responsive" src="/img/product-uploads/'.$item.'" id="prod_image_preview" width="20%" />';
...
//            }
            ...
            $img = $this->thumbnail($quotation_product['Product']['image']);
			// $data[$count][$key] = '<img class="img-responsive" src="/img/product-uploads/'.$item.'" id="prod_image_preview" width="20%" />';
